...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // ✅ Step 1: Login with OTP mock
  const loginStep1 = async (email: string, password: string, role: UserRole) => {
    const foundUser = users.find(u => u.email === email && u.role === role);
    ...
    console.info("💡 Demo OTP (check console):", otp);
  ...
  const verifyOtp = async (otp: string) => {
    ...
    if (otp !== savedOtp || !pendingEmail) throw new Error("Invalid OTP");

    const foundUser = users.find(u => u.email === pendingEmail);
  ...
  // Old login (direct, no OTP) – keep as fallback
  const login = async (email: string, password: string, role: UserRole): Promise<void> => {
    setIsLoading(true);
    try {
      await new Promise(resolve => setTimeout(resolve, 1000));
